...
trekmap.menu = {
	...
	show: function() {
	    ...
	    new Element('legend').setText('Tvorba trasy').injectTop(menu);
	    ...
	    $('trekmap-canvas').addClass('editable');
		return menu;
	},
	...
	hide: function() {
		...
		$('trekmap-canvas').removeClass('editable');
	},
...
trekmap.geocoding = {
	...
	/**
	 * Set directly and zoom.
	 */
	set: function(address) {
		return trekmap.geocoding.find(address, trekmap.geocoding.zoom);
	},
	 ...
	 */
	search: function() {
		var address = $$("#trekmap-geocoding input[type='text']").getProperty('value');
		return trekmap.geocoding.find(address, trekmap.geocoding.zoom);
	},
	...
	find: function(address, callback) {
		...
		geocoder.getLatLng(
			...
			function(point) {
				// bounds
				var sw = new GLatLng(trekmap.geocoding.bounds.sw[0], trekmap.geocoding.bounds.sw[1]);
				var ne = new GLatLng(trekmap.geocoding.bounds.ne[0], trekmap.geocoding.bounds.ne[1]);
				var bounds = new GBounds([sw, ne]);
				
				// point processing
				if (!point || !bounds.containsPoint(point)) {
					alert(address + ' nenalezeno'); // TODO dialog
				} else {
...
window.addEvent('domready', function () { if ($defined($('trekmap'))) {
    ...
    // geocoding
    $$('#trekmap-geocoding button').addEvent('click', trekmap.geocoding.search);
    $$("#trekmap-geocoding input[type='text']").addEvent('keydown', function(event) {
    	event = new Event(event);
    	if (event.key == 'enter') { trekmap.geocoding.search(); }
    } );
    ...
    if ($defined(window.php.autoload)) { // autoload
    	trekmap.track.decode(window.php.autoload);
    	trekmap.update();
    } else if ($defined(window.php.place)) { // auto zoom
    	trekmap.geocoding.set(php.place);
    	trekmap.update();
    }
} });
